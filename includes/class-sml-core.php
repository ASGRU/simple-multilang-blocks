    ...
    public function render_settings_page() {
        ...
        <?php if ( is_array( $wpml_preview ) && ! empty( $wpml_preview['error'] ) ) : ?><div class="notice notice-error inline"><p><?php echo esc_html( $wpml_preview['error'] ); ?></p></div><?php elseif ( is_array( $wpml_preview ) && ! empty( $wpml_preview['result'] ) && is_array( $wpml_preview['result'] ) ) : $preview = $wpml_preview['result']; ?>
            <div class="notice notice-info inline"><p><strong><?php esc_html_e( 'Preflight complete.', 'simple-multilang-blocks' ); ?></strong> <?php esc_html_e( 'Nothing was changed. Review the relationship counts before importing.', 'simple-multilang-blocks' ); ?></p><ul class="sml-migration-summary">
                ...
                <?php if ( ! empty( $preview['ignored_post_rows'] ) ) : ?><li><?php echo esc_html( sprintf( __( 'Ignored WPML post rows with missing or mismatched objects: %d', 'simple-multilang-blocks' ), absint( $preview['ignored_post_rows'] ) ) ); ?></li><?php endif; ?>
                <?php if ( ! empty( $preview['ignored_term_rows'] ) ) : ?><li><?php echo esc_html( sprintf( __( 'Ignored WPML term rows with missing objects: %d', 'simple-multilang-blocks' ), absint( $preview['ignored_term_rows'] ) ) ); ?></li><?php endif; ?>
                <?php if ( ! empty( $preview['conflicting_post_groups'] ) || ! empty( $preview['conflicting_term_groups'] ) ) : ?><li><?php echo esc_html( sprintf( __( 'Conflicting groups with duplicate language assignments: %d', 'simple-multilang-blocks' ), absint( $preview['conflicting_post_groups'] ?? 0 ) + absint( $preview['conflicting_term_groups'] ?? 0 ) ) ); ?></li><?php endif; ?>
