...
final class SML_Theme_Strings {
    ...
    public function register_page() {
        add_submenu_page(
            'options-general.php',
            __( 'Theme strings', 'simple-multilang-blocks' ),
            __( 'Theme strings', 'simple-multilang-blocks' ),
            'manage_options',
            'sml-theme-strings',
    ...
    public function filter_gettext( $translation, $text, $domain ) {
        if ( ! $this->is_theme_domain( $domain ) || '' === (string) $text ) {
            return $translation;
        }
    ...
    private function translated_value( $domain, $gettext_context, $text, $fallback ) {
        $string_id = SML_Core::find_string_id( self::string_context( $domain ), self::string_name( $gettext_context, $text ) );
        if ( ! $string_id ) {
            return $fallback;
        }
    ...
    public function render_page() {
        ...
        <div class="wrap sml-admin-wrap">
            <h1><?php esc_html_e( 'Theme strings', 'simple-multilang-blocks' ); ?></h1>
            <p class="description"><?php esc_html_e( 'The theme POT catalogue is used as the source. Translations are applied at runtime by the plugin, so the theme itself is never modified and no MO file is required.', 'simple-multilang-blocks' ); ?></p>
            <?php if ( isset( $_GET['scanned'] ) ) : ?><div class="notice notice-success"><p><?php esc_html_e( 'Theme strings were catalogued.', 'simple-multilang-blocks' ); ?></p></div><?php endif; ?>
            ...
            <div class="sml-toolbar">
                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                    ...
                    <button class="button button-secondary"><?php esc_html_e( 'Scan active theme POT', 'simple-multilang-blocks' ); ?></button>
                </form>
            ...
            <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                ...
                <table class="widefat fixed striped sml-strings-table">
                    ...
                    <tbody>
                    <?php if ( ! $rows ) : ?>
                        <tr><td colspan="<?php echo esc_attr( count( $languages ) + 1 ); ?>"><?php esc_html_e( 'No strings yet. Scan the active theme POT catalogue first.', 'simple-multilang-blocks' ); ?></td></tr>
                    <?php else : foreach ( $rows as $row ) : ?>
                        <tr>
                            <th scope="row"><code><?php echo esc_html( $row->source_value ); ?></code><?php if ( false !== strpos( $row->name, "\004" ) ) : ?><p class="description"><?php echo esc_html( substr( $row->name, 0, strpos( $row->name, "\004" ) ) ); ?></p><?php endif; ?></th>
        ...
        foreach ( $directories as $directory ) {
            ...
            foreach ( (array) $candidates as $file ) {
                foreach ( self::parse_pot( $file ) as $entry ) {
                    SML_Core::register_string( self::string_context( $domain ), self::string_name( $entry['context'], $entry['id'] ), $entry['id'] );
                }
            }
        }
    }
